...
struct SIMDGather {
    ...
    static void gather(TB* b, const int16_t* a, const TC* c, size_t buckets, int num_rows) {
        ...
        if constexpr (std::is_signed_v<TB>) {
            if constexpr (std::is_signed_v<TC>) {
                detail::gather_int16_buckets(reinterpret_cast<int32_t*>(b), a, reinterpret_cast<const int32_t*>(c),
                                             buckets, num_rows);
            } else {
                detail::gather_int16_buckets(reinterpret_cast<int32_t*>(b), a, reinterpret_cast<const uint32_t*>(c),
                                             buckets, num_rows);
            }
        } else {
            if constexpr (std::is_signed_v<TC>) {
                detail::gather_int16_buckets(reinterpret_cast<uint32_t*>(b), a, reinterpret_cast<const int32_t*>(c),
                                             buckets, num_rows);
            } else {
                detail::gather_int16_buckets(reinterpret_cast<uint32_t*>(b), a, reinterpret_cast<const uint32_t*>(c),
                                             buckets, num_rows);
            }
        }
    }
    ...
    static void gather(DataType* dest, const DataType* src, const IndexType* indexes, const CondType* is_filtered,
        ...
        if constexpr (std::is_trivially_copyable_v<DataType> && sizeof(IndexType) == 4 && sizeof(CondType) == 1) {
            if constexpr (std::is_signed_v<IndexType>) {
                detail::gather_bytes_filtered(dest, src, sizeof(DataType), reinterpret_cast<const int32_t*>(indexes),
                                              reinterpret_cast<const uint8_t*>(is_filtered), num_rows);
            } else {
                detail::gather_bytes_filtered(dest, src, sizeof(DataType), reinterpret_cast<const uint32_t*>(indexes),
                                              reinterpret_cast<const uint8_t*>(is_filtered), num_rows);
            }
