...
template <typename Elem, typename IndexType>
void gather_typed(void* dest, const void* src, const IndexType* indexes, size_t num_rows) {
    auto* d = reinterpret_cast<Elem*>(dest);
    const auto* s = reinterpret_cast<const Elem*>(src);
    size_t i = 0;
    ...
    if (num_rows >= 8192) {
        for (; i + kPrefetchDistance + kUnroll <= num_rows; i += kUnroll) {
            STARROCKS_PREFETCH(&s[indexes[i + kPrefetchDistance + 0]]);
            STARROCKS_PREFETCH(&s[indexes[i + kPrefetchDistance + 2]]);
            STARROCKS_PREFETCH(&s[indexes[i + kPrefetchDistance + 4]]);
            STARROCKS_PREFETCH(&s[indexes[i + kPrefetchDistance + 6]]);
...
void gather_bytes_impl(void* dest, const void* src, size_t elem_size, const IndexType* indexes, size_t num_rows) {
    switch (elem_size) {
    case 1:
        return gather_typed<uint8_t>(dest, src, indexes, num_rows);
    case 2:
        return gather_typed<uint16_t>(dest, src, indexes, num_rows);
    case 4:
        return gather_typed<uint32_t>(dest, src, indexes, num_rows);
    case 8:
        return gather_typed<uint64_t>(dest, src, indexes, num_rows);
    case 12:
        return gather_typed<Bytes<12>>(dest, src, indexes, num_rows);
    case 16:
        return gather_typed<Bytes<16>>(dest, src, indexes, num_rows);
    case 24:
        return gather_typed<Bytes<24>>(dest, src, indexes, num_rows);
    case 32:
        return gather_typed<Bytes<32>>(dest, src, indexes, num_rows);
    default:
...
template <typename Elem, typename IndexType>
void gather_filtered_typed(void* dest, const void* src, const IndexType* indexes, const uint8_t* is_filtered,
                           size_t num_rows) {
    auto* d = reinterpret_cast<Elem*>(dest);
    const auto* s = reinterpret_cast<const Elem*>(src);
    size_t i = 0;
        ...
        for (; i + kPrefetchDistance + kUnroll <= num_rows; i += kUnroll) {
            for (size_t k = 0; k < kUnroll; k += 2) {
                if (is_filtered[i + kPrefetchDistance + k] == 0) {
                    STARROCKS_PREFETCH(&s[indexes[i + kPrefetchDistance + k]]);
                }
            }

            d[i + 0] = (is_filtered[i + 0] == 0) ? s[indexes[i + 0]] : Elem{};
            d[i + 1] = (is_filtered[i + 1] == 0) ? s[indexes[i + 1]] : Elem{};
            d[i + 2] = (is_filtered[i + 2] == 0) ? s[indexes[i + 2]] : Elem{};
            d[i + 3] = (is_filtered[i + 3] == 0) ? s[indexes[i + 3]] : Elem{};
            d[i + 4] = (is_filtered[i + 4] == 0) ? s[indexes[i + 4]] : Elem{};
            d[i + 5] = (is_filtered[i + 5] == 0) ? s[indexes[i + 5]] : Elem{};
            d[i + 6] = (is_filtered[i + 6] == 0) ? s[indexes[i + 6]] : Elem{};
            d[i + 7] = (is_filtered[i + 7] == 0) ? s[indexes[i + 7]] : Elem{};
        }
    }
    ...
    for (; i < num_rows; ++i) {
        d[i] = (is_filtered[i] == 0) ? s[indexes[i]] : Elem{};
    }
}

template <typename IndexType>
void gather_filtered_default(void* dest, const void* src, size_t elem_size, const IndexType* indexes,
        ...
        for (; i + kPrefetchDistance + kUnroll <= num_rows; i += kUnroll) {
            ...
            for (size_t j = 0; j < kUnroll; ++j) {
                if (is_filtered[i + j] == 0) {
                    ...
                    std::memset(d + (i + j) * elem_size, 0, elem_size);
                }
            }
        }
    }
    ...
    for (; i < num_rows; ++i) {
        if (is_filtered[i] == 0) {
            ...
            std::memset(d + i * elem_size, 0, elem_size);
        }
    }
}

template <typename IndexType>
void gather_bytes_filtered_impl(void* dest, const void* src, size_t elem_size, const IndexType* indexes,
                                const uint8_t* is_filtered, size_t num_rows) {
    switch (elem_size) {
    case 1:
        return gather_filtered_typed<uint8_t>(dest, src, indexes, is_filtered, num_rows);
    case 2:
        return gather_filtered_typed<uint16_t>(dest, src, indexes, is_filtered, num_rows);
    case 4:
        return gather_filtered_typed<uint32_t>(dest, src, indexes, is_filtered, num_rows);
    case 8:
        return gather_filtered_typed<uint64_t>(dest, src, indexes, is_filtered, num_rows);
    case 12:
        return gather_filtered_typed<Bytes<12>>(dest, src, indexes, is_filtered, num_rows);
    case 16:
        return gather_filtered_typed<Bytes<16>>(dest, src, indexes, is_filtered, num_rows);
    case 24:
        return gather_filtered_typed<Bytes<24>>(dest, src, indexes, is_filtered, num_rows);
    case 32:
        return gather_filtered_typed<Bytes<32>>(dest, src, indexes, is_filtered, num_rows);
    default:
        return gather_filtered_default(dest, src, elem_size, indexes, is_filtered, num_rows);
    }
}
...
void gather_bytes_filtered(void* dest, const void* src, size_t elem_size, const uint32_t* indexes,
                           const uint8_t* is_filtered, size_t num_rows) {
    gather_bytes_filtered_impl(dest, src, elem_size, indexes, is_filtered, num_rows);
}

void gather_bytes_filtered(void* dest, const void* src, size_t elem_size, const int32_t* indexes,
                           const uint8_t* is_filtered, size_t num_rows) {
    gather_bytes_filtered_impl(dest, src, elem_size, indexes, is_filtered, num_rows);
}
